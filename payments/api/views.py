...
from ..models import Event, Customer, CurrentSubscription, EventProcessingException
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                (
                    ...
                    webhook_type,
                    webhook_livemode,
                ) = self.validate_webhook(webhook_data)
                ...
                if is_webhook_valid:
                    if Event.objects.filter(stripe_id=webhook_id).exists():
                        ...
                        event_processing_exception_serializer = (
                            EventProcessingExceptionSerializer(obj)
                        )
                        return Response(
                            event_processing_exception_serializer.data,
                        ...
                        return Response(
                            event_serializer.data, status=status.HTTP_200_OK
                        )
                else:
                    error_data = {
                        u"error": u"Webhook must contain id, type and livemode."
                    }
                    return Response(error_data, status=status.HTTP_400_BAD_REQUEST)
            else:
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
