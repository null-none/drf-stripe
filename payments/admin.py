...
def user_search_fields():  # coverage: omit
    User = get_user_model()
    fields = ["user__{0}".format(User.USERNAME_FIELD)]
...
class CustomerHasCardListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return [["yes", "Has Card"], ["no", "Does Not Have a Card"]]
...
class CustomerSubscriptionStatusListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        statuses = [
            [x, x.replace("_", " ").title()]
            for x in CurrentSubscription.objects.all()
...
admin.site.register(
    ...
    search_fields=["stripe_id", "customer__stripe_id", "validated_message"]
    + customer_search_fields(),
)
...
def customer_user(obj):
    ...
    return "{0} <{1}>".format(username, email)
