...
class Event(StripeObject):
    ...
    def validate(self):
        ...
        self.validated_message = json.loads(
            json.dumps(evt.to_dict(), sort_keys=True, cls=stripe.StripeObjectEncoder)
        )
    ...
    def process(self):  # @@@ to complex, fix later  # noqa
        ...
        if not self.valid or self.processed:
            return
        try:
            if not self.kind.startswith("plan.") and not self.kind.startswith(
                "transfer."
            ):
                self.link_customer()
            ...
            defaults.update(
                {
                    ...
                    "net": convert_amount_for_db(
                        summary.get("net"), transfer["currency"]
                    ),
                }
            )
        for field in defaults:
            if field.endswith("fees") or field.endswith("gross"):
                defaults[field] = convert_amount_for_db(
                    defaults[field]
                )  # assume in usd only
        if event.kind == "transfer.paid":
            defaults.update({"event": event})
            obj, created = Transfer.objects.get_or_create(
                stripe_id=transfer["id"], defaults=defaults
            )
        else:
            obj, created = Transfer.objects.get_or_create(
                stripe_id=transfer["id"], event=event, defaults=defaults
            )
...
class Customer(StripeObject):
    ...
    def sync(self, cu=None):
        ...
        if hasattr(cu, "active_card") and cu.active_card:
            ...
            if (
                ...
                or self.card_fingerprint != cu.active_card.fingerprint
                or self.card_kind != cu.active_card.type
            ):
                updated = True
                self.card_last_4 = cu.active_card.last4
    ...
    def sync_current_subscription(self, cu=None):
        ...
        if sub is None:
            ...
            try:
                ...
                sub_obj.amount = convert_amount_for_db(
    ...
    def charge(
        self, amount, currency="usd", description=None, send_receipt=True, capture=True
    ):
        """
    ...
    def sync_from_stripe_data(cls, stripe_invoice, send_receipt=True):
        ...
        invoice, created = cls.objects.get_or_create(
            ...
            defaults=dict(
                ...
                subtotal=convert_amount_for_db(
                    stripe_invoice["subtotal"], stripe_invoice["currency"]
                ),
                total=convert_amount_for_db(
                    stripe_invoice["total"], stripe_invoice["currency"]
                ),
        ...
        if not created:
            ...
            invoice.subtotal = convert_amount_for_db(
                stripe_invoice["subtotal"], stripe_invoice["currency"]
            )
            invoice.total = convert_amount_for_db(
                stripe_invoice["total"], stripe_invoice["currency"]
            )
        ...
        for item in stripe_invoice["lines"].get("data", []):
            ...
            if not inv_item_created:
                inv_item.amount = convert_amount_for_db(
...
class Charge(StripeObject):
    ...
    def refund(self, amount=None):
        ...
        charge_obj = stripe.Charge.retrieve(self.stripe_id).refund(
            amount=convert_amount_for_api(
                self.calculate_refund_amount(amount=amount), self.currency
            )
        )
    ...
    def sync_from_stripe_data(cls, data):
        ...
        if data.get("amount_refunded"):
            # pylint: disable=C0301
            obj.amount_refunded = convert_amount_for_db(
                data["amount_refunded"], obj.currency
            )
        if data["refunded"]:
